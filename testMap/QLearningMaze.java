...
public class QLearningMaze {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("【初期盤面】（○がスタート、☓が障害物、□がゴール）");
        env.printBoard();
        ...
        for (int i = 0; i < LENGTH; i++) {
            for (int j = 0; j < WIDTH; j++) {
                if (i == agentRow && j == agentCol) {
                    sb.append("○"); // エージェントの現在位置
                } else {
                    switch (map[i][j]) {
                        case 0: sb.append(" "); break; // 空白
                        case 2: sb.append("☓"); break; // 障害物
                        case 3: sb.append("□"); break; // ゴール
                    }
                }
            }
            sb.append("\n");
...
class Agent {
    ...
    public void test() {
        ...
        while (!done && stepCount < 20) { // 無限ループ防止のため最大20歩
